...
ref_wfm  = avrg - dc
ref_wfm /= max(ref_wfm)
#tes.plot.waveform( (time, ref_wfm), show=True )
...
for waveform in tes.waveform.read_binary( path=input, length=length): #, frame=10000):
	...
	#tes.plot.waveform( (time, dc_wfm, ref_wfm*e), show=True)
	#tes.plot.waveform( (time, dc_wfm, ref_wfm*e, ref_wfm*e2), show=True)
	tes.histogram.add( hist_raw, dc+dc_wfm[cut] )
